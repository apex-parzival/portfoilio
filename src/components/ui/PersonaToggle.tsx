...
export const PersonaToggle = ({ viewMode, onChange }: PersonaToggleProps) => (
    /*
     * Centred by a plain flex wrapper rather than `left-1/2 -translate-x-1/2`.
     * Framer writes its own inline `transform` for the entry animation, which
     * overrode the Tailwind translate class and left the control pinned at the
     * 50% mark instead of centred on it. It sat right of centre on desktop and
     * ran off the edge on a phone, clipped by the body's overflow-x.
     */
    <div className="fixed bottom-5 inset-x-0 z-40 flex justify-center px-4 pointer-events-none">
        <motion.div
            ...
            className="pointer-events-auto flex items-center gap-1 rounded-full border border-white/10 bg-background/85 backdrop-blur-md p-1"
            role="group"
            ...
            {options.map((option) => {
                ...
                return (
                    <button
                        ...
                        aria-pressed={active}
                        className={`px-3 sm:px-4 py-2 rounded-full text-[11px] font-bold tracking-widest uppercase whitespace-nowrap transition-colors duration-300 ${active
